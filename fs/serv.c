...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	struct OpenFile *f;
	int r = openfile_lookup(envid, req->req_fileid, &f);
	if(r < 0) return r;

	r = file_read(f->o_file, ret, req->req_n, f->o_fd->fd_offset);
	if(r < 0) return r;
	f->o_fd->fd_offset += r;
	return r;
}
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
	...
	r = file_write(f->o_file, req->req_buf, req->req_n, f->o_fd->fd_offset);
	if(r < 0) return r;
	f->o_fd->fd_offset += r;
	return r;
}
