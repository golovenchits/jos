...
mem_init(void)
{
	...
	mem_init_mp();
	boot_map_region(kern_pgdir, KERNBASE, 0xffffffff - KERNBASE - PGSIZE,0 , PTE_W | PTE_P);
...
page_init(void)
{
	...
	// free pages!
	size_t i;
	size_t first_extmem_free_page_num = (uint32_t)PADDR(boot_alloc(0)) / PGSIZE;
	...
	for (i = npages_basemem; i < first_extmem_free_page_num; i++){
		pages[i].pp_ref = 1;
	}

	for(i = first_extmem_free_page_num; i < npages; i++){
		pages[i].pp_ref = 0;
		pages[i].pp_link = page_free_list;
		page_free_list = &pages[i];
	}
...
{
	// Fill this function in
	for (uintptr_t i = va; i < va+size; i+=PGSIZE) {
		pte_t* pte = pgdir_walk(pgdir, (void*)i, true);
		*pte = pa | perm | PTE_P;
		pa += PGSIZE;
	}
}
...
{
	// Fill this function in
	pte_t* pte_lookup = pgdir_walk(pgdir, va, false);
	if (!pte_lookup || !(*pte_lookup & PTE_P))
		return NULL;
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	uintptr_t end = ROUNDDOWN((uintptr_t) va + len - 1, PGSIZE);
	...
	for (uintptr_t i = start; i <= end; i += PGSIZE) {
		...
		if (!page_lookup(env->env_pgdir, (void *) i, &pte_store) ||
		    (perm & ~(*pte_store & PTE_SYSCALL))) {
			user_mem_check_addr = i < (uintptr_t) va ? (uintptr_t) va : i;
			return -E_FAULT;
		}
	}
