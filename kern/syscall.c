...
sys_env_set_status(envid_t envid, int status)
{
	...
	int err = envid2env(envid, &new_env, 1);
	if(err < 0) {
		return err;
	}
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	err = page_insert(new_env->env_pgdir, page, va, perm);
	if(err){
		page_free(page);
		return err;
	}
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	if((perm & ~PTE_SYSCALL) || 
		((perm & (PTE_U | PTE_P)) != (PTE_U | PTE_P)) ||
		((uintptr_t)srcva >= UTOP) || PGOFF(srcva) ||
	...
	if(!page || ((perm & PTE_W) && !(*pte_store & PTE_W))) {
		return -E_INVAL;
	}

	err = page_insert(dst_env->env_pgdir, page, dstva, perm);
	if(err) {
		return err;
	}
